...
public class AutoCut {
    ...
    public static Bitmap crop(Bitmap srcBmp, Point[] cropPoints) {
        ...
        int cropWidth = (int) ((PointUtils.getPointsDistance(leftTop, rightTop)
                + PointUtils.getPointsDistance(leftBottom, rightBottom))/2);
        int cropHeight = (int) ((PointUtils.getPointsDistance(leftTop, leftBottom)
                + PointUtils.getPointsDistance(rightTop, rightBottom))/2);

        Bitmap cropBitmap = Bitmap.createBitmap(cropWidth, cropHeight, Bitmap.Config.ARGB_8888);
        AutoCut.nativeCrop(srcBmp, cropPoints, cropBitmap);
        return cropBitmap;
    }
